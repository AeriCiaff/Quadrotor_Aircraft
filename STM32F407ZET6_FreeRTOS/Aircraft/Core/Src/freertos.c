...
void MX_FREERTOS_Init(void) {
  ...
  osThreadDef(myTask03, OLED_Task, osPriorityBelowNormal, 0, 128);
...
void MPU6050_Task(void const * argument)
{
  ...
  for(;;)
  {
		if(MPU6050_DMP_GetData(&Angle.pitch,&Angle.roll,&Angle.yaw) == 0){
...
void BMP280_Task(void const * argument)
{
  ...
  for(;;)
  {
		...
		printf("T: %.2f  P: %f \r\n", BMP280Sensor.temp, BMP280Sensor.press);
    osDelay(200);
  }
...
void OLED_Task(void const * argument)
{
  ...
  for(;;)
  {
    OLED_ShowString(1,1,Yaw,16);
		OLED_ShowNum(33,1,Angle.yaw,5,16);
    OLED_ShowString(1,3,Pitch,16);
    OLED_ShowNum(49,3,Angle.pitch,5,16);
    OLED_ShowString(1,5,Roll,16);
    OLED_ShowNum(41,5,Angle.roll,5,16);
		osDelay(10);
  }
