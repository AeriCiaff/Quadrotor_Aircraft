...
void BMP280_Restart(void){
	...
	HAL_I2C_Mem_Write(&hi2c2, (BMP280_ADDR << 1), BMP280_RESET_REG, REG_SIZE, &reset_cmd, 1, 0xFFFF);
}

//修正参数读取函数
void BMP280_ReadParameter(void){
	uint8_t buffer[2];
	HAL_I2C_Mem_Read(&hi2c2, (BMP280_ADDR << 1), BMP280_REG_ADDR_LSB_T1, REG_SIZE, buffer, 2, 0xFFFF);
	BMP280.T1 = (uint16_t)(buffer[1] << 8 | buffer[0]);
	HAL_I2C_Mem_Read(&hi2c2, (BMP280_ADDR << 1), BMP280_REG_ADDR_LSB_T2, REG_SIZE, buffer, 2, 0xFFFF);
	BMP280.T2 = (uint16_t)(buffer[1] << 8 | buffer[0]); 
	HAL_I2C_Mem_Read(&hi2c2, (BMP280_ADDR << 1), BMP280_REG_ADDR_LSB_T3, REG_SIZE, buffer, 2, 0xFFFF);
	BMP280.T3 = (uint16_t)(buffer[1] << 8 | buffer[0]);
	HAL_I2C_Mem_Read(&hi2c2, (BMP280_ADDR << 1), BMP280_REG_ADDR_LSB_P1, REG_SIZE, buffer, 2, 0xFFFF);
	BMP280.P1 = (uint16_t)(buffer[1] << 8 | buffer[0]);	
	HAL_I2C_Mem_Read(&hi2c2, (BMP280_ADDR << 1), BMP280_REG_ADDR_LSB_P2, REG_SIZE, buffer, 2, 0xFFFF);
	BMP280.P2 = (uint16_t)(buffer[1] << 8 | buffer[0]);
	HAL_I2C_Mem_Read(&hi2c2, (BMP280_ADDR << 1), BMP280_REG_ADDR_LSB_P3, REG_SIZE, buffer, 2, 0xFFFF);
	BMP280.P3 = (uint16_t)(buffer[1] << 8 | buffer[0]);
	HAL_I2C_Mem_Read(&hi2c2, (BMP280_ADDR << 1), BMP280_REG_ADDR_LSB_P4, REG_SIZE, buffer, 2, 0xFFFF);
	BMP280.P4 = (uint16_t)(buffer[1] << 8 | buffer[0]);
	HAL_I2C_Mem_Read(&hi2c2, (BMP280_ADDR << 1), BMP280_REG_ADDR_LSB_P5, REG_SIZE, buffer, 2, 0xFFFF);
	BMP280.P5 = (uint16_t)(buffer[1] << 8 | buffer[0]);	
	HAL_I2C_Mem_Read(&hi2c2, (BMP280_ADDR << 1), BMP280_REG_ADDR_LSB_P6, REG_SIZE, buffer, 2, 0xFFFF);
	BMP280.P6 = (uint16_t)(buffer[1] << 8 | buffer[0]);
	HAL_I2C_Mem_Read(&hi2c2, (BMP280_ADDR << 1), BMP280_REG_ADDR_LSB_P7, REG_SIZE, buffer, 2, 0xFFFF);
	BMP280.P7 = (uint16_t)(buffer[1] << 8 | buffer[0]);
	HAL_I2C_Mem_Read(&hi2c2, (BMP280_ADDR << 1), BMP280_REG_ADDR_LSB_P8, REG_SIZE, buffer, 2, 0xFFFF);
	BMP280.P8 = (uint16_t)(buffer[1] << 8 | buffer[0]);
	HAL_I2C_Mem_Read(&hi2c2, (BMP280_ADDR << 1), BMP280_REG_ADDR_LSB_P9, REG_SIZE, buffer, 2, 0xFFFF);
...
void BMP280_Init(void){
	...
	HAL_I2C_Mem_Write(&hi2c2, (BMP280_ADDR << 1), BMP280_CTRL_MEAS_REG, REG_SIZE, &ctrl_meas_cmd, 1, 0xFFFF);
	HAL_I2C_Mem_Write(&hi2c2, (BMP280_ADDR << 1), BMP280_CONFIG_REG, REG_SIZE, &config_cmd, 1, 0xFFFF);
...
void BMP280_Measure_Cmd(void){
	...
	HAL_I2C_Mem_Write(&hi2c2, (BMP280_ADDR << 1), BMP280_CTRL_MEAS_REG, REG_SIZE, &measure_cmd, 1, 0xFFFF);
}
...
void BMP280_ReadData(void){
	...
	HAL_I2C_Mem_Read(&hi2c2, (BMP280_ADDR << 1), BMP280_PRESS_MSB_REG, REG_SIZE, data, 6, 0xFFFF);
